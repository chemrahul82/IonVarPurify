...
def bamrc(bamrcPath,regionFile,bam,refFasta):

    bname = os.path.basename(regionFile)
    command = '%s -q 30 -b 20 -f %s -d 2000 -l %s %s > out.%s 2>/dev/null' %(bamrcPath,refFasta,regionFile,bam,bname)
    os.system(command)

    
def execute_multiprocess(bamrcPath,regionFile,bam,refFasta,ncpu):
    ...
    jobs = []
    for i in chunks:
        p = multiprocessing.Process(target=bamrc,args=(bamrcPath,i,bam,refFasta))
        jobs.append(p)
        p.start()

...
def annovar_annotate(software_dir, annovar_dir, somatic_all_vcf, ncpu):

    """ Run annovar on potential variants to get annotations
    
    Input: 1. Path of the Somatic Calling software
    ...
    oldAnnoFile = '_tmp_/annovar_somatic_annotation_table.txt'
    oldAnnoMnpLog = '_tmp_/annovar_somatic_table_mnp.log'

    if not os.path.exists(oldAnnoFile):
        command = '%s/run_annovar.sh %s %s %s >/dev/null 2>&1' %(software_dir, annovar_dir, somatic_all_vcf, ncpu)
        os.system(command)
        AnnoFile = 'annovar_somatic_annotation_table.txt'
        AnnoMnpLog = 'annovar_somatic_table_mnp.log'
...
def damagingStats(annolist,idxlist,mnp_idx):
    ...
    for idx, el in enumerate(annolist):
        if el.split('\t')[5] == 'exonic' and ('nonsynonymous' in el.split('\t')[8] or 'stopgain' in el.split('\t')[8]):
...
def removeGerm(varList, annoList):
    ...
    for idx in range(len(annoList)):
        ...
        if (_1000g[idx] != '.' or dbsnp[idx] != '.'):
            ngermline += 1
            germ_maf.append(float(varList[idx].split('\t')[9]))
        else:
...
def rescue_cosmic(varListAll, annoListAll, oldIdxList, cosmic):
    ...
    Input: list of all potential variants with information from matched variant file
           ...
           list with indices of variants that map them with the original list
           list with indices of cosmic entries
    ...
    nvar0 = len(oldIdxList)

    varList = []; annoList = []
    #append the indices of cosmic variants that were filtered out by Fisher's test & germline removal to the index list
    for cosidx in cosmic:
        if cosidx not in oldIdxList:
            #varList.append(varListAll[cosidx])
            #annoList.append(annoListAll[cosidx])
            oldIdxList.append(cosidx)

    #sort the index list and remake the sorted variant and annovar lists
    idxList = sorted(oldIdxList,key=int)
    for i in idxList:
        varList.append(varListAll[i])
        annoList.append(annoListAll[i])
    ...
    #    if idx in idxList and idx not in cosmic and el.split('\t')[5] == 'exonic' and el.split('\t')[7] != 'synonymous_SNV'\

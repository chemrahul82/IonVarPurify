...
def process_somatic(tumorvcf, tumorbam, bedfile, bedHeader, blackfile, softdir, bamrcPath, ref_fasta, \
    ...
    # create output files
    filt_var_anno_file = open('Somatic_Germline_Filtered_Dups.tsv', 'w') # output file to write stage-1 filter-passed variants
    ...
    # rescue cosmic variants that fail Fisher's test: cosmic count > 0; this criteria will be more stringent during read QC
    #varList, annoList, idxList, tmp = rescue_cosmic(varListAll, annoListAll, varList, annoList, idxList, cosmic)
    varList, annoList, idxList, nwhite1 = rescue_cosmic(varListAll, annoListAll, idxList, cosmic)
    ...
    os.system("awk '!x[$0]++' %s > %s" %('Somatic_Germline_Filtered_Dups.tsv','Somatic_Germline_Filtered_noDups.tsv'))
    nDup = int(subprocess.check_output("grep -c '' Somatic_Germline_Filtered_Dups.tsv", shell=True)) - int(subprocess.check_output("grep -c '' Somatic_Germline_Filtered_noDups.tsv", shell=True))


    #-------------Change Diff_Q, Diff_MMQS, Diff_readlen1,Diff_pos column to '.' for HOMs with AF = 1 in T
    oldtempfile = open('Somatic_Germline_Filtered_noDups.tsv','r')
    newtempfile = open('Somatic_Germline_Filtered_temp.tsv','w')
    ...
    with open('Somatic_Germline_Filtered_temp.tsv','r') as f:
        next(f)
        all_maf = [float(line.split('\t')[8]) for line in f]
    
    newtempfile = open('Somatic_Germline_Filtered.tsv','w')
    newheader = header + ['Percentile_Score']
    newtempfile.write('\t'.join(newheader) + '\n')

    oldtempfile = open('Somatic_Germline_Filtered_temp.tsv','r')
    next(oldtempfile)
    ...
    #os.remove('Somatic_Germline_Filtered_Dups.tsv'); os.remove('Somatic_Germline_Filtered_noDups.tsv'); os.remove('Somatic_Germline_Filtered_temp.tsv')
    ...
    os.system('''awk '{if (NR>1) print $1"\t"$2"\t"$4"\t"$5}' Somatic_Germline_Filtered.tsv > temp1.region''')
    ...
    tempList = []
    with open('Somatic_Germline_Filtered.tsv','r') as f:
        next(f)
        for line in f:
            tempList.append(line.strip().split('\t'))

    of = open('Somatic_Germline_Filtered_Detailed.tsv','w')
    ...
    of.close()
    os.remove('Somatic_Germline_Filtered.tsv')
    os.system('mv Somatic_Germline_Filtered_Detailed.tsv Somatic_Germline_Filtered.tsv')
            ...
            if int(fields[10]) > 0:
                ...
                if (ncoshit > 0 and int(fields[16]) > ncoshit):
                    ...
                    if fields[-1] != 'amplicon_bias' and fields[-1] != 'mispriming':
                        #remove variants with zero allele depth
                        if int(fields[9]) > 0:
                            #treat homozygous
                            if float(fields[8]) == 1:
    ...
    nf = open('Somatic_QC_HP_Passed.tsv','w')
    nf.write('\t'.join(newheader1) + '\n')
    with open('Somatic_QC_Passed.tsv','r') as of:
        next(of)
        for line in of:
            #whitelist cosmic
            fields = line.strip().split('\t')
    ...
    #apply var allele depth filter
    nf = open('Somatic_QC_HP_VarDepth_Passed.tsv','w')
    nf.write('\t'.join(newheader1) + '\n')
    with open('Somatic_QC_HP_Passed.tsv','r') as of:
        next(of)
        for line in of:
            #whitelist cosmic
            fields = line.strip().split('\t')
    ...
    if blackfile != '':
        ...
        newfile1 = open('Somatic_HQ.tsv','w')
        ...
        with open('Somatic_QC_HP_VarDepth_Passed.tsv','r') as vf:
            next(vf)
            for line in vf:
        ...
        try:
            nfc1 = int(subprocess.check_output("grep -c '' Somatic_QC_HP_VarDepth_Passed.tsv", shell=True))
        except subprocess.CalledProcessError:
            nfc1 = 0
        try:
            nfc2 = int(subprocess.check_output("grep -c '' Somatic_HQ.tsv", shell=True))
        except subprocess.CalledProcessError:
            nfc2 = 0
        ...
        os.system('cp Somatic_QC_HP_VarDepth_Passed.tsv Somatic_HQ.tsv')
        nblack = 0
    ...
    if run_mode == 'paired':
        command = '''awk '{if (NR==1 || $7 == 0 || $17>=5) print}' Somatic_HQ.tsv > Somatic_UHQ.tsv'''
        os.system(command)
    else:
        command = 'cp Somatic_HQ.tsv Somatic_UHQ.tsv'
    ...
    return ngermline, nFishFail, nwhite1, nwhite, nblack, nDup, n_misprime, n_ampbias, nPCR_error, nvarDepth, nzeroVarDepth, n_hp_snp, n_hp_indel, n_hp_mnp, all_maf

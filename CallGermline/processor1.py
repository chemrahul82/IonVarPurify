...
def annovar_annotate(software_dir, annovar_dir, var_vcf, ncpu):
	...
	if not os.path.exists('annovar_variant_annotation_table.txt'):
		command = '%s/run_annovar.sh %s %s %s >/dev/null 2>&1' %(software_dir, annovar_dir, var_vcf, ncpu)
		os.system(command)
	else:
		print "	looks like annovar has already been run, delete the output file to rerun"

	
	with open('annovar_variant_annotation_table.txt', 'r') as annofile:
		next(annofile)
   ...
   #get the indices of variants with mnp and their annotation
	...
	with open('annovar_variant_table_mnp.log', 'r') as mnpfile:
		for line in mnpfile:
			if 'position' in line:
